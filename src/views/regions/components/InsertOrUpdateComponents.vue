    ...
    <v-card rounded="xl">
      <v-card-title class="pa-6 pb-3">
        <div class="d-flex align-center gap-2">
          <v-icon size="small" color="onPrimary">fas fa-map-marker</v-icon>
...
const emptyForm = () => ({ name: '', regionNumber: '', branchCount: 0 })
...
function openForEdit(item) {
  ...
  form.value = { name: item.name, regionNumber: item.regionNumber, branchCount: item.branchCount }
  dialog.value = true
}
...
function save() {
  if (isEditMode.value) {
    ...
    showSnackbar('تم تعديل المنطقة بنجاح')
  } else {
    store.add({ ...form.value, branchCount: 0 })
    showSnackbar('تمت إضافة المنطقة بنجاح')
  }
  close()
}
