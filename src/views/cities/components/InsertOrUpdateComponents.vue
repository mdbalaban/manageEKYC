...
const emptyForm = () => ({ name: '', countryId: null, population: '' })
...
function openForEdit(item) {
  ...
  form.value = { name: item.name, countryId: item.countryId, population: item.population }
  dialog.value = true
}
...
function save() {
  ...
  if (isEditMode.value) {
    ...
    store.add({ ...form.value, countryName: country?.name ?? '', regionsCount: 0 })
